...
import (
	...
	"golang.com/golang.com/weak_Password_Check/plugins"
...
func main() {
	...
	flag.IntVar(&port, "p", 3306, "Enter the port number, default is 3306")
	...
	for _, taskList := range ipList {
		...
		go func(s vars.Service) {
			...
			result, err := plugins.ScanMysql(s)
			if err != nil {
				if strings.Contains(err.Error(), "to database") { //有时存在部分数据库账户无法执行db.Ping()的情况，报错信息包含to database
	...
	wg.Wait()
	fmt.Println("All scans completed,The results have been written to Result.txt.")
}
